...
static const int ActivationsNumber = ACTIVATION_APPLY(SUM);


// Array of string of each Activation:
static const char *ActivationsStringArray[] = {ACTIVATION_APPLY(TO_STRING)};
...
Activation getActivation(char *string)
{
	int index = 0;
...
Number activation(Activation fun, Number x)
{
	switch (fun)
	{
		...
		case ELu:
			...
			return x >= 0 ? x : ELU_COEFF * (exp(x) - 1);
			break;

		case SELu:

			return x >= 0 ? SELU_COEFF_POS * x : SELU_COEFF_NEG * (exp(x) - 1);
			break;
...
Number der_activation(Activation fun, Number x)
{
	switch (fun)
	{
		...
		case ELu:
			...
			return x >= 0 ? 1 : ELU_COEFF * exp(x);
			break;

		case SELu:

			return x >= 0 ? SELU_COEFF_POS : SELU_COEFF_NEG * exp(x);
			break;
