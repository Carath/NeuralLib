...
void printNetwork(NeuralNetwork *network, PrintOption opt)
{
	...
	printf("\nThis network has learned: %d, and has %d neuron layer(s):\n\n", network -> HasLearned, network -> LayersNumber);
...
int saveNetwork(NeuralNetwork *network, char *foldername)
{
	...
	sprintf(folder_address, "%s/%s", SAVES_FOLDER_NAME, foldername);

	createFolder(SAVES_FOLDER_NAME); // In case /saves/ is missing.
	createFolder(folder_address);

	// Creating a text file with the network's readable data:

	char infos_filename[MAX_PATH_LENGTH];
	sprintf(infos_filename, "%s/infos.txt", folder_address);
	...
	for (int l = 0; l < network -> LayersNumber; ++l)
	{
		...
		sprintf(net_filename, "%s/net_%d.bin", folder_address, l);
